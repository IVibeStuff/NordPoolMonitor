...
# 3. Find installer
ls dist/
# → NordPool Monitor Setup 5.6.5.exe
```
...
```
dist/
├── NordPool Monitor Setup 5.6.5.exe   (~80 MB installer)
...
1. Upload `NordPool Monitor Setup 5.6.5.exe` to file host
...
gh release create v5.6.5 dist/*.exe
```
...
```bash
npm run build:mac
# → dist/NordPool Monitor-5.6.5.dmg
```

### **On Linux (to build Linux app):**
```bash
npm run build:linux
# → dist/NordPool Monitor-5.6.5.AppImage
```
