...
# 3. Find installer
ls dist/
# → NordPool Monitor Setup 5.5.0.exe
```
...
```
dist/
├── NordPool Monitor Setup 5.5.0.exe   (~80 MB installer)
...
1. Upload `NordPool Monitor Setup 5.5.0.exe` to file host
...
gh release create v5.5.0 dist/*.exe
```
...
```bash
npm run build:mac
# → dist/NordPool Monitor-5.5.0.dmg
```

### **On Linux (to build Linux app):**
```bash
npm run build:linux
# → dist/NordPool Monitor-5.5.0.AppImage
```
